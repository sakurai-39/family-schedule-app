# 家族スケジュール管理アプリ — AI実装憲法（AGENTS.md）
...
**Codex への注記:**
- このファイルは `CLAUDE.md` と同じ内容です。Codex は `AGENTS.md` を読み、Claude Code は `CLAUDE.md` を読みますが、内容は揃えてあります。両方のファイルを更新する場合は同じ内容にしてください。
- 計画書は `docs/superpowers/plans/` に保存されています。M6 → Plan 3 → Plan 4-8 の順に実装してください。
- 引き継ぎ時の現在地・進め方は `docs/codex-handoff-prompt.md` を参照。
